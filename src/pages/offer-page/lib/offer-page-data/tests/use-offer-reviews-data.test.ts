...
import { useOfferReviewsData } from '../use-offer-reviews-data';
...
describe('Hook \'useNearOffersData\'', () => {
  ...
  it('should return correct signature', () => {
    const { result } = renderHook(() => useOfferReviewsData(), { wrapper: storeWrapper });
  ...
  it('should return near offers from store', () => {
    ...
    expect(result.current).toEqual(PAGE_STATE_MOCK.comments);
  });
});
